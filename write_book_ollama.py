...
"model": os.environ.get("BOOK_WRITER_MODEL", "ollama/gemma3:4b"),
    "proxy_host": os.environ.get("BRIGHT_DATA_HOST", "brd.superproxy.io"),
    "proxy_port": int(os.environ.get("BRIGHT_DATA_PORT", "33335")),
    "proxy_username": os.environ.get("BRIGHT_DATA_USERNAME", "<Get from brightdata.com in SERP API>"),
    "proxy_password": os.environ.get("BRIGHT_DATA_PASSWORD", "<Get from brightdata.com in SERP API>"),
...
class BrightDataWebSearchTool(BaseTool):
    ...
    def _run(self, query: str, num_results: int = CONFIG["search_results_count"]) -> List[SearchResult]:
        """Run the web search and return results."""
        try:
            ...
            response = requests.get(url, proxies=proxies, verify=False, timeout=CONFIG["timeout_seconds"])
...
def create_research_agent(llm):
    ...
    return Agent(
        ...
        goal="Research the topic {topic} and collect comprehensive information about it.",
        backstory="You are a {topic} expert with a deep understanding of the subject. "
...
def create_outline_agent(llm):
    ...
    return Agent(
        ...
        goal="Create a detailed and well-structured outline for a book about {topic}",
...
def create_chapter_writer_agent(llm):
    ...
    return Agent(
        role="Chapter Writer",
        goal="Write an informative and engaging chapter for a book about {topic}",
        backstory="You are a professional writer with expertise in {topic}. "
...
class BookFlow(Flow[BookState]):
    ...
    async def save_book(self):
        ...
        base_filename = f"{safe_title}_{date_str}"
